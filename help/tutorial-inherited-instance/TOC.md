---
user-guide-title: チュートリアル – 継承されたMarketo Engageインスタンスを監査する際のヒントとテクニック
user-guide-description: 継承されたMarketo Engageインスタンスの監査に関するヒントとテクニック
breadcrumb-title: チュートリアル – 継承されたMarketo Engageインスタンスを監査する際のヒントとテクニック
breadcrumb-url: /docs/marketo-learn/auditing-an-inherited-instance/overview.html
index: false
role: Admin
level: Beginner
auto-video-transcripts: true
source-git-commit: c2b949e268e66a0d85c251658a49048e451fac1e
workflow-type: tm+mt
source-wordcount: '77'
ht-degree: 0%

---


# チュートリアル – 継承されたMarketo Engageインスタンスを監査する際のヒントとテクニック {#auditing-an-inherited-instance}

+ [チュートリアル – 概要](/help/tutorial-inherited-instance/overview.md)
+ [継承されたインスタンスの監査](/help/tutorial-inherited-instance/audit-an-inherted-instance.md)
+ [CRM 同期エラーをログに記録して、トラブルシューティングを容易にする](/help/tutorial-inherited-instance/log-crm-sync-errors-for-easy-troubleshootig.md)
+ [インスタンスガバナンスガイドの作成](/help/tutorial-inherited-instance/develop-an-instance-governance-guide.md)
+ [マーケティングテクニカルスタックを視覚的にマッピング](/help/tutorial-inherited-instance/create-a-visual-data-flow-diagram.md)
+ [プログラムの事前開始品質保証プロセス](/help/tutorial-inherited-instance/essential-program-pre-launch-qa.md)
+ [Marketo EngageTutorials](https://experienceleague.adobe.com/docs/marketo-learn/tutorials/overview.html?lang=ja)
