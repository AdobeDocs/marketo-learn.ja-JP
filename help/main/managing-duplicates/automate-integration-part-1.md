---
title: 統合による重複管理の自動化 – 第1部 – iPaaS ツールのレビュー
description: VertifyやWorkatoなどのiPaaS ツールとAPI統合を利用して、Marketo Engageで重複管理を効率化する方法をご確認ください。
role: Admin
level: Intermediate, Experienced
doc-type: Feature Video
duration: 0
last-substantial-update: 2025-01-10T00:00:00Z
jira: KT-16746
exl-id: d23c31e0-8bf5-42d1-a167-6122670068ca
source-git-commit: 62ea57d025da4f21bc289ce2d454b51e9e2ff415
workflow-type: tm+mt
source-wordcount: '58'
ht-degree: 0%

---

# 統合による重複管理の自動化 – 第1部 – iPaaS ツールのレビュー

VertifyやWorkatoなどのiPaaS ツールとAPI統合を利用して、Marketo Engageで重複管理を効率化する方法をご確認ください。

>[!VIDEO](https://video.tv.adobe.com/v/3441918/?captions=jpn&learn=on&enablevpops)
