---
title: Marketo Engageでの重複の一括結合 – 第3部 – データの再インポート
description: 一括結合を完了するには、クリーンなデータをMarketo Engageに再インポートし、運用中のキャンペーンで重複を効果的に管理します。
role: Admin
level: Intermediate, Experienced
doc-type: Feature Video
duration: 0
last-substantial-update: 2025-01-10T00:00:00Z
jira: KT-16745
hidefromtoc: true
exl-id: 7204b255-40e4-4ba9-9b36-d4d4fbbd8a6c
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '58'
ht-degree: 0%

---

# Marketo Engageでの重複の一括結合 – 第3部 – データの再インポート

一括結合を完了するには、クリーンなデータをMarketo Engageに再インポートし、運用中のキャンペーンで重複を効果的に管理します。

>[!VIDEO](https://video.tv.adobe.com/v/3452748/?captions=jpn&learn=on&enablevpops)
