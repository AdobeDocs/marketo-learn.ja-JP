---
title: web パーソナライゼーションによる訪問者のエンゲージメントについて詳しく見る
description: 訪問者が誰であり、何をしているのかに基づいて、個々のニーズに即したエクスペリエンスを提供します。 web サイト訪問者をセグメンテーションおよびターゲティングして、よりカスタマイズされた、有意義で魅力的な体験を構築する方法をご確認ください。
role: User
level: Beginner
jira: KT-10725
thumbnail: 345405.jpeg
exl-id: bc772311-71f5-4b54-a5f2-c88a04621965
feature: Web Personalization
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '84'
ht-degree: 0%

---

# web パーソナライゼーションによる訪問者のエンゲージメントについて詳しく見る

訪問者が誰であり、何をしているのかに基づいて、個々のニーズに即したエクスペリエンスを提供します。 web サイト訪問者をセグメンテーションおよびターゲティングして、よりカスタマイズされた、有意義で魅力的な体験を構築する方法をご確認ください。

>[!VIDEO](https://video.tv.adobe.com/v/3411948/?captions=jpn&quality=12&learn=on){transcript=true}
