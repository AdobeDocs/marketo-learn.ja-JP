---
title: モバイルマーケティングとMarketoの統合方法
description: バイヤーペルソナの作成、KPIの設定、コミュニケーションの頻度の決定、 [!DNL Marketo’s]  マーケティングカレンダーの使用など、モバイルマーケティング戦略の改善方法をご覧ください。
role: User
level: Beginner
jira: KT-10729
thumbnail: 345402.jpeg
exl-id: 8e82a586-2001-4b7e-a2d4-073e78045854
feature: Mobile Marketing
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '58'
ht-degree: 0%

---

# Marketoとモバイルマーケティングを統合する

バイヤーペルソナの作成、KPIの設定、コミュニケーションの頻度の決定、[!DNL Marketo] マーケティングカレンダーの使用方法などを確認して、モバイルマーケティング戦略を改善する方法をご覧ください。

>[!VIDEO](https://video.tv.adobe.com/v/345402/?quality=12&learn=on){transcript=true}
