---
title: 自動トリガーによるメールキャンペーンの作成について詳しく見る
description: ' [!DNL Marketo] を使用して、トリガーされたメールキャンペーンを使用して、タイムリーで関連性の高いメッセージをエンゲージし、購入者の行動を最大限に活用する方法を説明します。'
role: User
level: Beginner
jira: KT-10762
thumbnail: 345479.jpeg
exl-id: 6c4cdcca-703e-41e2-a6e7-2ddba152e458
feature: Smart Campaigns
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '53'
ht-degree: 0%

---

# 自動トリガーによるメールキャンペーンの作成について詳しく見る

[!DNL Marketo]を使用して、トリガーされたメールキャンペーンを使用して、タイムリーで関連性の高いメッセージをエンゲージし、購入者の行動を活用する方法を説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3409790/?captions=jpn&quality=12&learn=on){transcript=true}
