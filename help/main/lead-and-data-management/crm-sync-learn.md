---
title: ' [!DNL Marketo] をCRMに同期する方法について説明します'
description: ' [!DNL Marketo] CRM Syncを使用すると、 [!DNL Marketo] とCRMの間で関連情報をシームレスに共有することにより、マーケティング活動とセールス活動を調整できます。  [!DNL Marketo] をCRMに同期する方法について説明します。'
role: User
level: Beginner
jira: KT-10744
thumbnail: 345428.jpeg
exl-id: 18512aac-f8f5-4b8a-8d94-2d5324089be0
feature: Getting Started
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '60'
ht-degree: 0%

---

# [!DNL Marketo]をCRMに同期する方法について説明します

[!DNL Marketo] CRM Syncを使用すると、[!DNL Marketo]とCRMの間で関連情報をシームレスに共有することにより、マーケティング活動とセールス活動を調整できます。 [!DNL Marketo]をCRMに同期する方法について説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3410454/?captions=jpn&quality=12&learn=on){transcript=true}
