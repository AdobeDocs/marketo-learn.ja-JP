---
title: ' [!DNL Salesforce]へのセールス アクティビティ ログの設定'
description: ' [!DNL Salesforce]へのセールス アクティビティ ログの設定'
role: User
level: Beginner
exl-id: a48410aa-cd68-46b2-bd52-1a7253470e74
feature: Sales Insight Actions
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '36'
ht-degree: 0%

---

# [!DNL Salesforce]へのセールス アクティビティ ログの設定 {#configure-sales-activity-logging-to-salesforce}

Sales Insight Actions アクティビティを設定して[!DNL Salesforce] アカウントに記録する方法について説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3449200/?captions=jpn&quality=12&learn=on){transcript=true}

## 関連動画

* [Sales Insightのアクションの概要](/help/main/sales-insight-actions/sales-insight-actions-overview.md){target="_blank"}
* [&#x200B; [!DNL Sales Insight Actions]  インスタンスへのアクセス](/help/main/sales-insight-actions/accessing-your-sales-insight-actions-instance.md){target="_blank"}
