...
### &#x200B;1. システム管理者と IT チームを特定し、必要なアクションについて話し合う {#identify-your-system-administrators}

* 組織内のシステム管理者が誰であるかわからない場合は、アドビのアカウントチームまたはアドビサポート `marketocares@marketo.com` にお問い合わせください。

* Marketo Engage サブスクリプションの移行先となる Adobe Admin Console（またはAdobe 組織）を確認します。Marketo Engage のネイティブ対話自動処理ツールである [Dynamic Chat](/help/dynamic-chat/dynamic-chat-overview.md){target="_blank"} 用の Adobe Admin Console を使用している方も多いでしょう。Marketo Engage サブスクリプションは、Dynamic Chat と同じ組織にデプロイする必要があります。
...
### &#x200B;2. Adobe ID への移行による変更点と影響について理解を深める {#familiarize-yourself-with-the-changes}
...
* [ユーザ設定チェックリスト](https://experienceleague.adobe.com/ja/docs/marketo/using/getting-started/initial-setup/user-setup){target="_blank"}
...
* [Adobe Admin Console への Marketo サブスクリプションとユーザの移行について](https://experienceleague.adobe.com/ja/docs/marketo/using/product-docs/administration/marketo-with-adobe-identity/subscription-and-user-migration/understanding-marketo-subscription-and-user-migration-to-the-adobe-admin-console){target="_blank"}
...
* [Adobe Admin Console の使用方法について](https://helpx.adobe.com/jp/enterprise/using/admin-console.html){target="_blank"}

### &#x200B;3. 内部チームに必要な移行タイムラインと準備を知らせる {#announce-the-migration-timeline}
...
i. リンクが記載されたメール確認リクエストメールが届きます（有効期限は 3 日間です）。リンクの有効期限が切れている場合は、Marketo Engage 内から「マイプロファイル」アイコンをクリックして、**マイアカウント**／**アカウント設定**／**確認を再送信**&#x200B;に移動すると、確認メールを再送信できます。
...
i. 「招待状に同意」ボタンをクリックし、Adobe ID を使用してログインすることで、新しい招待を受け入れます。
